...
public final class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        StorageLayout.ensure(this); StorageLayout.appendLog(this, "DonutHLE main menu opened"); CompatibilityLog.recordStartup(this); showHome();
    }
    @Override protected void onResume() { super.onResume(); if (!isFinishing()) showHome(); }
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (imported == null) { StorageLayout.appendLog(this, "APK import failed: " + source); Toast.makeText(this, "APK import failed. Check log.", Toast.LENGTH_LONG).show(); }
        else { StorageLayout.appendLog(this, "APK imported: " + imported.getName()); Toast.makeText(this, "Imported " + imported.getName(), Toast.LENGTH_SHORT).show(); }
        showHome();
    }
    private void showHome() {
        StorageLayout.ensure(this); LinearLayout content = column(); content.setPadding(dp(22), dp(20), dp(22), dp(28));
        TextView brand = label("DONUTHLE", 27, teal); brand.setTypeface(null, 1); content.addView(brand); content.addView(label("Android 1.6 • Donut • HLE prototype", 14, muted), margins(0, 2, 0, 22));
        TextView status = label("READY TO EXPLORE", 12, Color.rgb(112, 214, 164)); status.setTypeface(null, 1); content.addView(status);
        TextView title = label("Your games,\nyour sandbox.", 31, text); title.setTypeface(null, 1); content.addView(title, margins(0, 5, 0, 8)); content.addView(label("Import legal APK files and keep each game's files isolated in its own sandbox.", 15, muted), margins(0, 0, 0, 22));
        content.addView(card("▣", "GAME LIBRARY", "Import APK files into DonutHLE_apps", "IMPORT APK", v -> showLibrary()), margins(0, 0, 0, 10)); content.addView(card("⌁", "GAME SANDBOX", "Browse save data and per-game files", "OPEN FOLDER", v -> openFolder(StorageLayout.sandbox(this))), margins(0, 0, 0, 10)); content.addView(card("≡", "EMULATOR LOG", "Read compatibility and unimplemented-feature reports", "VIEW LOG", v -> showLog()), margins(0, 0, 0, 10));
        File[] apks = StorageLayout.apks(this); TextView library = label(apks.length == 0 ? "APK LIBRARY IS EMPTY\nUse IMPORT APK to add a game." : apks.length + " APK" + (apks.length == 1 ? "" : "S") + " READY\n" + apkNames(apks), 13, apks.length == 0 ? muted : text); library.setPadding(dp(14), dp(13), dp(14), dp(13)); library.setBackgroundColor(panel); content.addView(library, margins(0, 12, 0, 18));
        Button options = button("OPTIONS  ›", false); options.setOnClickListener(v -> showOptions()); content.addView(options, margins(0, 0, 0, 8)); Button about = button("ABOUT DONUTHLE  ›", false); about.setOnClickListener(v -> showAbout()); content.addView(about); setContentView(scroll(content));
    }
    ...
    private View card(String icon, String heading, String description, String action, View.OnClickListener listener) { LinearLayout row = new LinearLayout(this); row.setOrientation(LinearLayout.HORIZONTAL); row.setGravity(Gravity.CENTER_VERTICAL); row.setPadding(dp(14), dp(14), dp(12), dp(14)); row.setBackgroundColor(panel); row.setOnClickListener(listener); TextView symbol = label(icon, 28, teal); symbol.setGravity(Gravity.CENTER); row.addView(symbol, new LinearLayout.LayoutParams(dp(40), dp(60))); LinearLayout copy = column(); copy.setPadding(dp(12), 0, dp(8), 0); TextView h = label(heading, 13, teal); h.setTypeface(null, 1); copy.addView(h); copy.addView(label(description, 15, text), margins(0, 3, 0, 0)); copy.addView(label(action, 11, muted), margins(0, 7, 0, 0)); row.addView(copy, new LinearLayout.LayoutParams(0, -2, 1)); row.addView(label("›", 28, muted)); return row; }
    private void showLibrary() { LinearLayout content = page("GAME LIBRARY", "Import APKs or place them in DonutHLE_apps."); content.addView(button("＋  IMPORT APK", true, v -> importApk()), margins(0, 18, 0, 12)); File[] apks = StorageLayout.apks(this); if (apks.length == 0) content.addView(label("No APKs found yet.\n\nUse IMPORT APK, or copy an .apk file into DonutHLE_apps and return here.", 16, muted), margins(0, 0, 0, 14)); else for (File apk : apks) content.addView(gameRow(apk), margins(0, 0, 0, 10)); content.addView(button("OPEN DONUTHLE_APPS  ›", false, v -> openFolder(StorageLayout.apps(this))), margins(0, 8, 0, 8)); content.addView(button("‹  BACK", false, v -> showHome())); setContentView(scroll(content)); }
    private View gameRow(File apk) { LinearLayout row = new LinearLayout(this); row.setOrientation(LinearLayout.VERTICAL); row.setPadding(dp(16), dp(14), dp(16), dp(14)); row.setBackgroundColor(panel); TextView name = label(apk.getName(), 16, text); name.setTypeface(null, 1); row.addView(name); row.addView(label(formatBytes(apk.length()) + " • Android package", 13, muted), margins(0, 4, 0, 10)); row.addView(button("▶  RUN GAME", true, v -> launchApk(apk))); return row; }
    ...
    private void openFolder(File folder) { StorageLayout.appendLog(this, "Opening file browser: " + folder.getName()); Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT_TREE); intent.putExtra("android.provider.extra.INITIAL_URI", DocumentsContract.buildRootUri("org.donuthle.android.documents", "donuthle")); startActivity(intent); }
    private void launchApk(File apk) { StorageLayout.appendLog(this, "LAUNCH REQUEST: " + apk.getName()); CompatibilityLog.inspectApk(this, apk); Toast.makeText(this, "Logged missing implementations for " + apk.getName(), Toast.LENGTH_LONG).show(); showLog(); }
    private void showLog() { StorageLayout.appendLog(this, "Opened emulator log"); LinearLayout content = page("EMULATOR LOG", "UTF-8 trace • unimplemented features are marked UNIMPLEMENTED"); TextView log = label(StorageLayout.readLog(this) + "\n" + CompatibilityLog.statusText() + "\n" + nativeRuntimeInfo().replace("\\n", "\n"), 14, text); log.setTextIsSelectable(true); log.setTypeface(android.graphics.Typeface.MONOSPACE); log.setPadding(dp(14), dp(14), dp(14), dp(14)); log.setBackgroundColor(panel); content.addView(log, margins(0, 16, 0, 16)); content.addView(button("‹  BACK", true, v -> showHome())); setContentView(scroll(content)); }
