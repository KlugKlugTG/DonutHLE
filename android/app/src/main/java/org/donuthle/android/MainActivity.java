...
public final class MainActivity extends Activity {
    ...
    private View card(String icon, String heading, String description, String action, View.OnClickListener listener) {
        ...
        TextView headingView = label(heading, 13, teal);
        headingView.setTypeface(null, 1);
    ...
    private void showLibrary() {
        LinearLayout content = page("GAME LIBRARY", "Import APKs or place them in DonutHLE_apps.");
        content.addView(button("＋  IMPORT APK", true, v -> importApk()), margins(0, 18, 0, 12));
        File[] apks = StorageLayout.apks(this);
        if (apks.length == 0) {
            content.addView(label("No APKs found yet.\n\nUse IMPORT APK, or copy an .apk file into DonutHLE_apps and return here.", 16, muted), margins(0, 0, 0, 14));
        } else {
    ...
    private View gameRow(File apk) {
        ...
        row.addView(name);
        row.addView(label(formatBytes(apk.length()) + " • Android package", 13, muted), margins(0, 4, 0, 10));
        row.addView(button("▶  RUN GAME", true, v -> launchApk(apk)));
        return row;
    }
    ...
    private void openFolder(File folder) {
        ...
        try {
            ...
            StorageLayout.appendLog(this, "UNIMPLEMENTED/ERROR: no DocumentsUI provider: " + error.getMessage());
            Toast.makeText(this, "File browser unavailable; use IMPORT APK.", Toast.LENGTH_LONG).show();
        }
    }

    private void launchApk(File apk) {
        StorageLayout.appendLog(this, "LAUNCH REQUEST: " + apk.getName());
        CompatibilityLog.inspectApk(this, apk);
        Toast.makeText(this, "Compatibility scan saved to log. Runtime launch is not implemented yet.", Toast.LENGTH_LONG).show();
        showLog();
    }

    private void showLog() {
        StorageLayout.appendLog(this, "Opened emulator log");
        LinearLayout content = page("EMULATOR LOG", "UTF-8 trace • unimplemented features are marked UNIMPLEMENTED");
        TextView log = label(StorageLayout.readLog(this) + "\n" + CompatibilityLog.statusText() + "\n" + nativeRuntimeInfo(), 14, text);
    ...
    private void showOptions() {
        ...
        content.addView(button("OPEN LOG", false, v -> showLog()), margins(0, 0, 0, 8));
        content.addView(button("‹  BACK", true, v -> showHome()));
        setContentView(scroll(content));
    }
    ...
    private LinearLayout page(String heading, String description) {
        ...
        TextView headingView = label(heading, 28, text);
        headingView.setTypeface(null, 1);
        content.addView(headingView, margins(0, 28, 0, 4));
